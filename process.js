...
function getJobs() {
        ...
        try {
          let check=true;
           ...
           check=false;
            ...
            config={metalpath:'/var/www/html/node_modules/',baseURL: job.websitejobqueuedata.baseURL }
            let partialstotal = []
            let pageSeoTitle;
    ...
    q.on('progress',async (queueId, jobId, percent) => {
      ...
      var bar = new ProgressBar('  Progress [:bar]  :percent  ', {
        ...
        width:50,
        incomplete: ' ',
        total: rawConfigs[1].pageSettings.length
      });
     
      bar.tick(i)
      if(i==rawConfigs[1].pageSettings.length){
        i=1
      }else{
